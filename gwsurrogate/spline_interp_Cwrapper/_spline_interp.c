...
int spline_interp(const long data_size, const long out_size,
    ...
    gsl_interp *interp    = gsl_interp_alloc(gsl_interp_cspline, data_size);
    gsl_interp_init(interp, data_x, data_y, data_size);
    ...
    for (ii=0; ii < out_size; ii++) {
      status = gsl_interp_eval_e(interp, data_x, data_y,
                                 out_x[ii], acc, out_y+ii);
      if ( status ) {
        // error, bail out
        break;
      }
