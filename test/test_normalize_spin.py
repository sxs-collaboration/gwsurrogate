...
def test_positive_chi_norm():
    ...
    normalize_spin(chi, 0.5)
    norms = np.linalg.norm(chi, axis=1)
    np.testing.assert_allclose(norms, 0.5)


def test_zero_chi_norm():
    chi = np.array([[1.0, 2.0, 3.0]])
    expected = np.copy(chi)
    normalize_spin(chi, 0.0)
    np.testing.assert_array_equal(expected, chi)


def test_negative_chi_norm():
    chi = np.array([[1.0, 2.0, 3.0]])
    expected = np.copy(chi)
    normalize_spin(chi, -1.0)
    np.testing.assert_array_equal(expected, chi)
...
def test_direction_preserved():
    ...
    for i in range(len(chi)):
        expected_dir = original_chi[i] / np.linalg.norm(original_chi[i])
        result_dir = chi[i] / np.linalg.norm(chi[i])
        np.testing.assert_allclose(result_dir, expected_dir)


def test_single_row():
    chi = np.array([[1.0, 0.0, 0.0]])
    normalize_spin(chi, 0.8)
    np.testing.assert_allclose(chi, [[0.8, 0.0, 0.0]])
